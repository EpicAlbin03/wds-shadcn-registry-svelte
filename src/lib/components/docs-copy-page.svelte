<script lang="ts">
	...
	function getPromptUrl(baseURL: string) {
		return `${baseURL}?q=${encodeURIComponent(
			`I’m looking at this wds-shadcn-registry-svelte documentation: ${pageUrl}.
...
{#snippet Trigger({ props }: { props: PropsType })}
	<Button
		...
		variant="secondary"
		size="sm"
		class={cn('peer -ml-0.5 size-8 shadow-none md:size-7 md:text-[0.8rem]', props.class as string)}
	>
